...
function sacaTono(nota, frecuencia){
	...
	for (var i = 0; i < arrayFrecuencias.length; i++) {
		...
		console.log(arrayFrecuencias[i]+"|"+nota+"|"+frecuencia);
...
function notaComprobador(notaAfinador, notaPartitura){
	...
	if (notaAfinador.trim() == notaPartitura.trim()) {
		return true;
	}
...
function actualizarAfinador() {
	...
	document.getElementById(divAfinador).innerHTML = notaTono; //Esto hay que repararlo
	...
	//console.log(notaComprobador(notaTono, notaPartitura)+"| "+notaTono+"| "+notaPartitura);
...
//console.log(nota+"|"+notaPartitura+"|");
